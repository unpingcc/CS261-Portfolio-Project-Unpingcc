...
# Assignment: 6
# Due Date: 03/11/2022
# Description: Portfolio Project Assignment 6 Hash_Map using LinkedLists
...
class HashMap:
    ...
    def get(self, key: str) -> object:
        ...
        """
        #develop hash based off of key and create index for list
        hash_it = self.hash_function(key)
        index = hash_it % self.capacity

        alist = self.buckets.get_at_index(index)
        #if list contains the key iterate though alist and return the LL value
    ...
    def put(self, key: str, value: object) -> None:
        ...
        if index < self.capacity:
            ...
            if self.contains_key(key):  # a key is in the Hash remove key and replace the key and value
                for node in alist:
                    alist.remove(key)
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        for item in range (self.capacity):
            alist = self.buckets.get_at_index(item)
            if alist.length()== None:
                continue
            for node in alist:
                #hash generator and inseration into new map
                hash_it = self.hash_function(node.key)
                index = hash_it % new_capacity
                new_hash_map.get_at_index(index).insert(node.key, node.value)
        self.buckets = new_hash_map
